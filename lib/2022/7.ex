...
aoc 2022, 7 do
  ...
  def p2 do
    ...
    unused_space = 70000000 - get_dir_size(fs, "/")
  ...
  defp get_fs_map() do
    ...
    |> Enum.reduce({%{"/" => %{dirs: [], files: []}}, ["/"]}, fn line, {fs, path = [_ | rest_of_path]} ->
      # IO.inspect(line)
      ...
      case line do
        ...
        ["dir", d] ->
          ...
          {Map.replace!(fs, current, Map.replace!(fs[current], :dirs, [dir_name | fs[current][:dirs]])), path}
        [size, filename] ->
          {Map.replace!(fs, current, Map.replace!(fs[current], :files, [%{name: filename, size: String.to_integer(size)} | fs[current][:files]])), path}
      end
    end)
    fs
  end
